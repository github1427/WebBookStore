...
<form action="<c:url value='/admin/AdminCategoryServlet'/>" method="post" onsubmit="return checkForm()">
    <input type="hidden" name="method" value="updateTwoLevel"/>
    ...
    <c:forEach items="${categories}" var="categoryele">
        <c:choose>
            <c:when test="${category.parent.cid eq categoryele.cid}">
                <option value="${categoryele.cid}" selected="selected">${categoryele.cname}</option>
            </c:when>
            <c:otherwise>
                <option value="${categoryele.cid}">${categoryele.cname}</option>
            </c:otherwise>
        </c:choose>
    </c:forEach>
</select><br/>
